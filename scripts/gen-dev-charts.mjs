...
const t = "motionGrid"; // will replace with div
const D = "div";

const file = `import { useMemo, useState, type ReactNode } from "react";
  ...
  return (
    ...
    >
      ...
      <${D} style={{ position: "relative", zIndex: 1 }}>
        <${D} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
          <${D}>
            <${D} style={{ fontSize: 11, fontWeight: 900, letterSpacing: 2.2, color: accent, textTransform: "uppercase" }}>{title}</${D}>
            {subtitle && <${D} style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 4 }}>{subtitle}</${D}>}
          </${D}>
          {right}
        </${D}>
        {children}
...
export function MetricTile({ label, value, sub, accent = "#00E5FF", delta }: {
  ...
  return (
    <${D} style={{
      ...
      <${D} style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", letterSpacing: 1.6, fontWeight: 800, textTransform: "uppercase" }}>{label}</${D}>
      <${D} style={{ fontSize: 22, fontWeight: 900, color: accent, marginTop: 4, fontVariantNumeric: "tabular-nums" }}>{value}</${D}>
      {(sub || delta) && (
        <${D} style={{ fontSize: 10, color: "rgba(255,255,255,0.55)", marginTop: 4 }}>
...
export function CollapsibleList({ title, count, defaultOpen = false, accent = "#00E5FF", children }: {
  ...
  return (
    <${D} style={{ border: \`1px solid \${accent}33\`, borderRadius: 10, overflow: "hidden" }}>
      ...
      {open && <${D} style={{ padding: 10, maxHeight: 360, overflowY: "auto" }}>{children}</${D}>}
    </${D}>
  );
}
...
export function DataTable({ columns, rows }: { columns: { key: string; label: string; width?: string }[]; rows: Record<string, ReactNode>[] }) {
  return (
    <${D} style={{ overflowX: "auto", borderRadius: 8, border: "1px solid rgba(0,229,255,0.2)" }}>
...
fs.writeFileSync(path.join(__dirname, "../src/utils/devAnalytics/devCharts.tsx"), file);
console.log("ok");
